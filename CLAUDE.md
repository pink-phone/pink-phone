...
**Build version.** Every image carries `APP_VERSION` + `APP_COMMIT` (Dockerfile `ARG`s, default `dev` / empty). The GitHub release workflow passes `gh-X.Y.Z` and the commit SHA of this repository (the `gh-` prefix marks a public build). It is shown in Settings → About (web bundle vs server, with a warning when they differ, e.g. a stale service-worker cache), returned by `GET /api/version` (public), and written to `/version.json` (no-cache, not precached by the service worker). Builds made outside a pipeline are `dev`.
