...
CI/CD via **Forgejo Actions** (`.forgejo/workflows/`), modeled on `../a reverse proxy`: `ci.yml` (non-`main` push → build+push both images `:<sha>`), `release.yml` (`main` push → semver patch bump, push `:latest`+`:<version>`, push git tag `vX.Y.Z`), `deploy.yml` (tag `v*` → SSH to the server, scp `deploy/docker-compose.yml`, `docker compose pull && up -d`). Two images at `pinkphone/pinkphone-{api,web}` (amd64). The **web image** (`Dockerfile`) is nginx serving the PWA **and** reverse-proxying `/api` → the api service, so production is same-origin (no CORS; built with `VITE_API_URL=""` → relative `/api`). `deploy/docker-compose.yml` runs db + api + web; only `web` is exposed (behind a reverse proxy for domain+TLS). The server `.env` is **regenerated and overwritten on every deploy** from the Forgejo secrets/vars (Forgejo is the source of truth, not the server file) — so keep `JWT_SECRET` stable there (changing it invalidates all sessions) and make sure all four `OIDC_*` values are set or SSO silently disables (`oidc_enabled()` needs them non-empty). See `deploy/README.md` for the Forgejo secrets/vars to configure.
