import type { Resources } from "./fr";

// Dictionnaire EN — doit refléter toutes les clés de fr.ts.
export const en: Resources = {
  common: {
    cancel: "Cancel",
    close: "Close",
    back: "Back",
    logout: "Log out",
    loading: "Loading…",
    loadMore: "Show more",
    send: "Send",
    save: "Save",
    edit: "Edit",
    delete: "Delete",
    actions: "Actions",
    unread: "Unread",
    alreadyRead: "Already seen",
  },
  app: {
    name: "Pink Phone",
  },
  nav: {
    aria: "Main navigation",
    dashboard: "Home",
    blog: "Blog",
    challenges: "Dares",
    desires: "Desires",
  },
  splash: {
    default: "One moment…",
    loadingSpace: "Loading your space…",
  },
  push: {
    unsupported: "Push isn't supported on this device.",
    denied: "Notification permission denied.",
    noKeys: "The server has no push keys configured.",
  },
  errors: {
    generic: "something went wrong",
    serverUnreachable:
      "Couldn't reach the server. SSO may be temporarily unavailable — reload the page.",
    ssoFailed: "SSO sign-in failed. Try again.",
  },
  auth: {
    subtitleSso: "Sign in to come in.",
    subtitleRegister: "Build your nest.",
    subtitleLogin: "Welcome back.",
    sso: "🔐 Sign in with SSO",
    or: "or",
    email: "Email",
    emailPlaceholder: "you@example.com",
    firstName: "Your first name",
    firstNamePlaceholder: "Alex",
    password: "Password",
    passwordHint: "8 characters minimum.",
    submitRegister: "Create my account",
    submitLogin: "Sign in",
    toggleToLogin: "Already have an account? Sign in",
    toggleToRegister: "No account yet? Create one",
  },
  onboarding: {
    welcome: "Welcome",
    welcomeName: "Welcome, {{name}}",
    subtitle: "Create your space, or join your partner's.",
    createTitle: "Create a space",
    spaceNameLabel: "Space name",
    create: "Create",
    joinTitle: "Join a space",
    spaceIdLabel: "Invite code",
    joinPlaceholder: "paste the code shared by your partner",
    joinHint: "Your partner generates it from their space home.",
    join: "Join",
  },
  dashboard: {
    today: "Today",
    settings: "Settings",
    you: "You",
    noMoodYet: "No mood yet today.",
    setMoodHint: "Tap to share your mood",
    editMoodAria: "Share or change my mood",
    moodHidden: "Your turn: share your mood to reveal theirs.",
    updatedAt: "Updated {{time}}",
    waitingPartnerTitle: "Waiting for your partner",
    waitingPartnerText: "Generate an invite code and share it so they can join you.",
    createInvite: "Create an invite code",
    inviteHint: "Single-use, valid 7 days. Share it with your partner.",
    moodQuestion: "How are you feeling?",
    moodSharedWith: "Your mood is shared with {{name}}.",
    moodSharedGroup: "Your mood is shared with the space.",
    moodSaved: "Your mood is saved.",
    moodRenews: "It refreshes every day.",
    moodPrompt: "Pick your mood of the day to send a sign.",
    newHeader: "What's new",
    newPosts_one: "{{count}} new message",
    newPosts_other: "{{count}} new messages",
    newComments_one: "{{count}} new comment",
    newComments_other: "{{count}} new comments",
    newChallenges_one: "{{count}} new dare",
    newChallenges_other: "{{count}} new dares",
    noticesSection: "Space activity",
  },
  notice: {
    memberJoined: "{{name}} joined the space.",
    downloadEnabled: "{{name}} enabled media downloads.",
  },
  moods: {
    aria: "My mood right now",
    calm: "Calm",
    flirty: "Playful",
    veryHot: "Very hot",
    tired: "Tired",
    cuddleNeeded: "Need some tenderness",
    custom: "Free mood",
    or: "or",
    addFree: "Free mood",
    addEmojiAria: "Free mood emoji",
    addLabelAria: "Free mood label",
    addPlaceholder: "🙂",
    labelPlaceholder: "a word…",
    customActiveAria: "Free mood — edit",
    removeAria: "Remove my mood",
  },
  reactions: {
    heart: "Heart",
    fire: "Hot",
    smirk: "Naughty",
    breath: "Breathless",
    hush: "Our secret",
    addAria: "Add a reaction (emoji)",
    addPlaceholder: "🙂",
    byOtherAria: "{{label}} — partner's reaction",
  },
  blog: {
    title: "Our journal",
    compose: "✍️ Write",
    empty: "Nothing yet. Share the first secret…",
    draftBadge: "Draft",
    draftsToggle_one: "Draft ({{count}})",
    draftsToggle_other: "Drafts ({{count}})",
    deletePost: "Delete this post",
    confirmDelete: "Delete this post? This is permanent.",
    publishDraft: "Publish this draft",
    edit: "Edit",
    edited: "edited",
    seenByPartner: "Seen",
    seenByAria: "See who has read it",
    seenByTitle: "Seen by",
    commentsLeave: "Leave a word",
    commentsCount_one: "{{count}} comment",
    commentsCount_other: "{{count}} comments",
    sharedPhotoAlt: "Shared media",
    favoriteAddAria: "Add to favorites",
    favoriteRemoveAria: "Remove from favorites",
    favoritesFilter: "★ Favorites",
    favoritesEmpty: "No favorites yet. Tap ★ on a post to keep it here.",
  },
  desires: {
    title: "Bucket list",
    intro:
      "Your list of desires to explore. Secretly tick what tempts you: it's only revealed on a match. Mark “✓ Done” what you've tried.",
    dashboardEntry: "Bucket list",
    dashboardHint: "Your desires to explore, in secret",
    matchBadge: "Match!",
    limitBadge: "❌ Limit",
    interestAria: "I'm tempted",
    removeInterestAria: "Actually, no",
    againstAria: "I'm against this (set a limit)",
    removeAgainstAria: "Remove my limit",
    doneBadge: "✓ Done",
    doneAria: "Mark as done",
    undoneAria: "Unmark “done”",
    emptyCategory: "Nothing in this category.",
    back: "Back",
    categories: {
      tender: "Tender & close",
      games: "Games & scenarios",
      positions: "Positions",
      sensations: "Sensations",
      power: "Power & roles",
      places: "Places & thrills",
      practices: "Practices (hot)",
    },
    items: {
      morningCuddle: "A cuddly wake-up",
      oilMassage: "An oil massage",
      bathTogether: "A bath together",
      slowDance: "An impromptu slow dance",
      lingerie: "Surprise lingerie or outfit",
      rediscover: "Rediscover each other slowly",
      roleplay: "A role play",
      truthOrDare: "Naughty truth or dare",
      striptease: "A striptease",
      photoSession: "A playful photo session",
      writeFantasy: "Write a fantasy together",
      daySexting: "Tease by text during the day",
      missionary: "Missionary",
      doggy: "Doggy style",
      cowgirl: "Cowgirl (her on top)",
      reverseCowgirl: "Reverse cowgirl",
      spooning: "Spooning",
      lotus: "The lotus",
      wheelbarrow: "The wheelbarrow",
      standing: "Standing, against a wall",
      edgeOfBed: "At the edge of the bed",
      mirror: "In front of a mirror",
      newPositionMonthly: "A new position to try",
      blindfold: "Blindfolded",
      lightBondage: "Light bondage",
      spanking: "A spanking",
      temperature: "Temperature play (ice, soft wax)",
      edging: "Slow teasing, holding back",
      toy: "A toy, an accessory",
      gentleDomination: "Gentle domination",
      submission: "Letting go, submitting",
      commands: "Give or follow commands",
      softCollar: "A soft collar",
      eveningRules: "An evening with rules",
      outdoors: "Outdoors",
      car: "In the car",
      hotel: "A hotel, a getaway",
      anotherRoom: "A room other than the bedroom",
      semiPublic: "The thrill of a semi-public place",
      voyeur: "Show off, film yourselves",
      fellatio: "Fellatio / blowjob",
      cunnilingus: "Cunnilingus",
      sixtyNine: "Sixty-nine",
      mutualMasturbation: "Mutual masturbation",
      fingering: "Fingering",
      rimming: "Rimming",
      anal: "Anal sex",
      facial: "Facial",
      swallow: "Swallowing",
      squirting: "Squirting",
    },
    itemsSoft: {
      fellatio: "A little oral treat",
      cunnilingus: "Oral pleasure for her",
      sixtyNine: "Pleasure each other at once",
      mutualMasturbation: "Touch each other",
      fingering: "Intimate caresses",
      rimming: "A very intimate kiss",
      anal: "Love from behind",
      facial: "Finish on her",
      swallow: "Go all the way",
      squirting: "Take her over the edge",
    },
  },
  eveningMenu: {
    title: "Tonight's menu",
    dashboardEntry: "Tonight's menu",
    dashboardHint: "What tempts you tonight, in secret",
    matchToast: "Match tonight!",
    intro:
      "Secretly tick what tempts you tonight. It's only revealed on a match. Resets each day.",
    matchCount_one: "✨ {{count}} match",
    matchCount_other: "✨ {{count}} matches",
    pickAria: "Pick for tonight",
    removeAria: "Remove from tonight",
    items: {
      cuddle: "Cuddles under the covers",
      movie: "A movie, snuggled up",
      candlelight: "Candlelit dinner",
      bath: "A bath together",
      massage: "A massage",
      slowDance: "A slow dance, just us",
      game: "A playful game",
      roleplay: "A role play",
      newThing: "Try something new",
      passionate: "A passionate night",
    },
  },
  loveNotes: {
    title: "Love notes",
    sheetTitle: "A love note",
    compose: "＋ Drop a note",
    composerLabel: "Leave a little note",
    placeholder: "Thinking of you…",
    schedule: "Schedule",
    scheduleAria: "Open date and time",
    send: "Send",
    empty: "No notes yet. Leave them a little surprise. 🌸",
    from: "— {{name}}",
    sealedTitle: "A surprise awaits you",
    opensAt: "Opens {{date}}",
    scheduledFor: "for {{date}}",
    deleteAria: "Delete this note",
  },
  postComposer: {
    sheetWrite: "Write",
    sheetEdit: "Edit draft",
    titleLabel: "Title (optional)",
    titlePlaceholder: "That idea on my mind…",
    bodyLabel: "Your story",
    bodyPlaceholder: "Tell it, take your time…",
    photoOptional: "Photo or video (optional)",
    mediaLabel: "Media (up to 10, reorder them)",
    moveUp: "Move up",
    moveDown: "Move down",
    attachFile: "Add a photo or video",
    photoChange: "Photo or video (change)",
    ephemeralAttached: "✦ Ephemeral media already attached",
    attachedAlt: "Attached media",
    remove: "Remove",
    removePhoto: "Remove the media",
    willRemove: "The media will be removed on save.",
    undo: "Undo",
    ephemeralToggle: "Ephemeral (view once)",
    ephemeralHint: "The media vanishes after a single view.",
    downloadToggle: "Downloadable",
    downloadHint: "Your partner will be able to save this media.",
    previewAlt: "Preview",
    publish: "Publish",
    saveDraft: "Save draft",
    saveEdits: "Save changes",
    publishing: "Publishing…",
    saving: "Saving…",
  },
  challenges: {
    title: "Our dares",
    empty: "No dares yet. Send the first one… 🔥",
    new: "＋ Send a dare",
    bank: "📚 Bank",
    deleteChallenge: "Delete this dare",
    confirmDelete: "Delete this dare? This is permanent.",
    deadline: "⏳ {{label}}",
    sections: {
      challengeAccepted: "In progress",
      proposed: "Proposals",
      maybeMaybe: "To adjust",
      jobDone: "Done",
    },
    intensity: {
      soft: "Soft",
      hot: "Hot",
      hard: "Hard",
    },
    status: {
      proposed: { label: "Proposed", hint: "Awaiting a reply…" },
      challengeAccepted: { label: "In progress", hint: "Challenge accepted ✨" },
      maybeMaybe: { label: "To adjust", hint: "Maybe, maybe… let's talk" },
      jobDone: { label: "Done", hint: "Job done — validated together 🎉" },
    },
    action: {
      accept: "🔥 Challenge accepted",
      negotiate: "Maybe, maybe…",
      maybeAccept: "Actually, I'm in",
      done: "✅ Job done",
    },
  },
  challengeComposer: {
    sheetTitle: "Send a dare",
    sheetEdit: "Edit dare",
    inspiration: "Inspiration (the bank)",
    reshuffle: "🎲 Other ideas",
    titleLabel: "Dare title",
    titlePlaceholder: "An oil massage…",
    descriptionLabel: "Description",
    descriptionPlaceholder: "What you propose, and how.",
    deadlineLabel: "Deadline (optional)",
    deadlinePlaceholder: "Before Sunday",
    submit: "🔥 Propose ({{intensity}})",
    intensityLabel: "Intensity",
  },
  challengeBank: {
    title: "Dare bank",
    intro: "Tap « ⋯ » to dare your partner with an idea, or manage your own.",
    empty: "No suggestions of your own yet.",
    add: "＋ Add",
    addTitle: "New idea",
    editTitle: "Edit idea",
    propose: "Dare my partner",
    fork: "Copy to my ideas",
    delete: "Delete",
    edit: "Edit",
    catalogue: "All suggestions",
    mine: "My suggestions",
    ownTag: "Yours",
    done: "Already done",
    hiddenTag: "Hidden",
    hide: "Hide",
    unhide: "Unhide",
  },
  comments: {
    sheetTitle: "Talk",
    empty: "Nobody reacted yet. Start the conversation.",
    replyLabel: "Your reply",
    replyPlaceholder: "A word, a desire…",
    loadOlder: "Older messages",
    actions: "Comment actions",
    editLabel: "Edit your comment",
    editFailed: "Couldn't save. Try again.",
    confirmDelete: "Delete this comment?",
  },
  settings: {
    title: "Settings",
    logoutAll: "Sign out of all devices",
    logoutAllHint:
      "If your device is lost or stolen: invalidates every session, everywhere (including here). Sign-in required.",
    logoutAllConfirm:
      "Sign out of all devices? Every session will be invalidated and you'll need to sign in again.",
    notifications: "Notifications",
    pushUnavailable:
      "Push isn't available on this device — daily digest or ghost mode are still possible.",
    pushModeTitle: "Instant notifications",
    pushModeDesc: "A discreet alert as soon as they post.",
    digestModeTitle: "Daily digest",
    digestModeDesc: "A single email per day, no interruptions.",
    soon: "soon",
    ghostModeTitle: "Ghost mode",
    ghostModeDesc: "No notifications, you discover by opening the app.",
    appearance: "Appearance",
    theme: "Theme",
    themeFelted: "Felted",
    themeRedVelvet: "Red Velvet",
    hotAnim: "Ember effect 🔥",
    hotAnimHint:
      "Animated halo and particles on hot states. Turn off for a calmer look.",
    language: "Language",
    reactionsSection: "Reactions",
    blindMood: "Mutual surprise",
    blindMoodHint:
      "Neither of you sees the other's mood before sharing your own. Both reveal at once.",
    allowMediaDownload: "Downloadable media",
    allowMediaDownloadHint:
      "Default for new posts: let your partner save media. Adjustable per post when publishing; no effect on ephemeral media.",
    desiresEnabled: "Bucket list",
    desiresEnabledHint:
      "Turn on the bucket list (“Desires” tab): each of you secretly ticks desires; they reveal on a match, and you can set limits.",
    desiresExplicit: "Explicit labels",
    desiresExplicitHint:
      "Bucket list: direct, blunt labels (“Fellatio”, “Anal sex”…). Turn off for a more suggestive register.",
    eveningMenuEnabled: "Tonight's menu",
    eveningMenuEnabledHint:
      "Turn on the evening ritual on the home screen: each of you secretly ticks what tempts you “for tonight”; it's revealed on a match. Resets each day.",
    reactionsIntro: "Choose the space's reactions and their order.",
    reactionEnable: "Enable reaction",
    reactionDisable: "Disable reaction",
    allowCustom: "Free reactions",
    allowCustomHint: "Allow a free emoji reaction via the ＋ button.",
    spacesSection: "My spaces",
    spacesHint: "{{count}} spaces",
    spaceCurrent: "current",
    spaceCreate: "Create another space",
    spaceCreateAction: "Create",
    spaceJoin: "Join a space",
    spaceJoinAction: "Join",
    accountSection: "My account",
    displayName: "Your name",
    spaceSection: "Our space",
    spaceName: "Space name",
    save: "Save",
    timezone: "Timezone",
    timezoneHint: "Used for the daily mood reset at midnight.",
    inviteId: "Invite ID",
    members: "Members",
    inviteMember: "Invite a member",
    securitySection: "Security",
    aboutSection: "About",
    releaseNotes: "What's new",
    releaseNotesHint: "See the latest app updates.",
    releaseNotesNew: "New",
  },
  releaseNotes: {
    title: "What's new",
    latest: "New",
    empty: "No release notes yet.",
  },
  lock: {
    title: "Pink Phone is locked",
    subtitle: "Enter your code to continue",
    wrong: "Wrong code. Try again.",
    mismatch: "The codes don't match.",
    delete: "Delete",
    settingTitle: "Passcode lock",
    settingOn: "On — code required when reopening.",
    settingOff: "Off — the app opens without a code.",
    enable: "Enable passcode",
    disable: "Disable",
    change: "Change code",
    setTitle: "Choose a code",
    setSubtitle: "4 digits, easy to remember",
    confirmTitle: "Confirm the code",
    confirmSubtitle: "Enter it once more",
    disableTitle: "Enter your current code",
    disableSubtitle: "To turn off the lock",
    biometric: "Unlock with biometrics",
    bioSettingTitle: "Biometric unlock",
    bioSettingOn: "On — FaceID / fingerprint when reopening (code stays the fallback).",
    bioSettingOff: "Use FaceID / Touch ID / fingerprint instead of the code.",
    bioEnable: "Enable biometrics",
    bioDisable: "Disable biometrics",
    bioFailed: "Couldn't enable biometrics. Try again.",
    bioPromptTitle: "Add biometric unlock?",
    bioPromptSubtitle:
      "Unlock with FaceID / Touch ID. The code stays as fallback if biometrics fail.",
    bioPromptEnable: "Enable now",
    bioPromptSkip: "Not now",
    bioHint: "FaceID / Touch ID becomes available once the passcode is set.",
  },
  install: {
    title: "Install Pink Phone",
    android: "On your home screen, like a real app.",
    ios: 'Tap ⎋ Share, then "Add to Home Screen".',
    install: "Install",
  },
  safeMedia: {
    secret: "Our secret",
    holdToReveal: "Press and hold to reveal",
    holdToWatch: "Press and hold to watch",
    ephemeralBadge: "✦ Ephemeral · once only",
    loading: "Loading…",
    unavailable: "Media unavailable — press and hold again to retry",
    goneTitle: "Gone…",
    goneText: "This memory only existed for a moment.",
    revealAria: "{{alt}} — hold to reveal",
    consumedAria: "Ephemeral media already viewed",
    download: "Download media",
    mute: "Mute",
    unmute: "Unmute",
  },
};
