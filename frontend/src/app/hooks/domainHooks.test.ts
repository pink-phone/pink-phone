...
describe("usePosts — pagination et mutations complémentaires", () => {
  ...
  it("openComments renverse l'ordre newest-first → oldest-first", async () => {
  ...
  it("loadMoreComments préfixe les anciens commentaires et met à jour commentsHasMore", async () => {
    a.listComments
      .mockResolvedValueOnce({
    ...
    const { result } = renderHook(() => usePosts("s1"));
    await act(async () => { await result.current.openComments("p1"); });
    // Après reverse: [c2, c3]. oldest = c2.createdAt = "t2"
  ...
  it("add renvoie true et préfixe le nouveau post", async () => {
    ...
    let ok = false;
    await act(async () => {
      ok = await result.current.add({ body: "bonjour", draft: false, viewOnce: false });
    });
    expect(ok).toBe(true);
  ...
  it("add renvoie false si l'API échoue et laisse la liste vide", async () => {
    ...
    let ok = true;
    await act(async () => {
      ok = await result.current.add({ body: "x", draft: false, viewOnce: false });
    });
    expect(ok).toBe(false);
